 ...
 * The icon set.
 *
 * Line icons on a 24×24 grid, 1.75px stroke, round caps and joins — one
 * geometry for all of them, which is what makes a set look drawn rather than
 * collected. They inherit `currentColor`, so an icon in a button, a tab bar
 * and a table row is the same file at three colours.
 *
 * NO EMOJI. The mockup uses 📶 💳 📋 for products, and emoji are the wrong
 * tool for a money app twice over: they render as a different picture on
 * every platform — so the eSIM tile a designer approved on a Mac is a
 * different glyph on the Android phone most Nigerian customers hold — and
 * they carry a colour and a shine that cannot be restyled, so they can never
 * sit quietly next to a typeface.
 */
...
/** Every path is drawn on the same 24×24 grid so the set reads as one hand. */
...
/** Icons that are closed shapes rather than strokes. */
const FILLED: ReadonlySet<IconName> = new Set(['grid']);
