...
function pages(dir: string, prefix: string): readonly string[] {
  const here = readdirSync(dir).includes('page.tsx') ? [prefix] : [];
  const below = readdirSync(dir).flatMap((entry) => {
    ...
    return pages(path, `${prefix}/${entry}`);
  });
  return [...here, ...below];
}
...
function linksIn(dir: string): readonly string[] {
  return readdirSync(dir).flatMap((entry) => {
    const path = join(dir, entry);
    if (statSync(path).isDirectory()) return linksIn(path);
